  ...
  return new Promise((resolve, reject) => {
    ...
    const opts = {
      ...
      headers: {
        ...
        'User-Agent': 'Shieldly-GitHubAction/1.2.0',
...
async function analyzeFile(filePath, hint, apiKey, apiUrl) {
  ...
  if (hint === 'cf') {
    ...
    if (isYaml) {
      // YAML files at this point are assumed to be CF templates; let the API validate
      policyType = 'cf';
      label = 'CloudFormation Template';
    } else {
      try {
  ...
  if (!apiKey && policyType === 'iam_identity') {
    // Demo mode: route through the web proxy (no auth required, IAM-only, per-IP capped)
    endpoint = 'https://www.shieldly.io/api/demo/analyze-iam';
    bodyPayload = { policy: fileContent, policyType };
  } else {
    if (apiKey) headers.Authorization = `Bearer ${apiKey}`;
    endpoint = policyType === 'cf' ? `${apiUrl}/v1/analyze/cf` : `${apiUrl}/v1/analyze/iam`;
    bodyPayload =
      policyType === 'cf' ? { template: fileContent } : { policy: fileContent, policyType };
  }

  let { status, data } = await request('POST', endpoint, bodyPayload, headers);
...
function buildPrComment({ filePath, label, score, riskLevel, findings, aiGrade, apiUrl }) {
  ...
  if (!hasFindings) {
    ...
    for (const f of findings) {
      const rem = (f.remediation || '').replace(/\|/g, '\\|').substring(0, 120);
      const title = (f.title || f.description || '').replace(/\|/g, '\\|');
      const resource = (f.resource || '—').replace(/\|/g, '\\|');
      lines.push(
...
function buildMultiPrComment(results, scanPath, apiUrl) {
  ...
  if (totalFindings === 0) {
    ...
    for (const f of sorted) {
      const rem = (f.remediation || '').replace(/\|/g, '\\|').substring(0, 100);
      const title = (f.title || f.description || '').replace(/\|/g, '\\|');
      const resource = (f.resource || '—').replace(/\|/g, '\\|');
...
async function run() {
  try {
    ...
    if (!apiKey) core.info('Mode: demo (5 lifetime analyses — set api-key for full access)');
