...
/** Map a Shieldly severity to a SARIF result level. */
...
/** GitHub code scanning severity score (security-severity rule property). */
...
/** Stable-ish rule id from a finding title: lowercase, alphanumeric + dashes. */
function sarifRuleId(finding) {
  const base = String(finding.title || finding.description || 'shieldly-finding')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .substring(0, 80);
  return `shieldly/${base || 'finding'}`;
}

/**
 * Build a SARIF 2.1.0 log from analysis results so findings can be uploaded to
 * GitHub code scanning (repository Security tab) via codeql-action/upload-sarif.
 * Line numbers are not available from the analysis, so every result anchors to
 * line 1 of its file.
 */
function buildSarif(results) {
  ...
  for (const r of results) {
    // Code scanning requires artifact URIs relative to the repo root, with
    // forward slashes and no leading "./".
    const uri = path.relative(process.cwd(), path.resolve(r.filePath)).split(path.sep).join('/');
    ...
    for (const f of r.findings) {
      ...
      if (!rulesById.has(ruleId)) {
        rulesById.set(ruleId, {
          id: ruleId,
          name: (f.title || f.description || 'Shieldly finding').substring(0, 200),
          shortDescription: {
            text: (f.title || f.description || 'Shieldly finding').substring(0, 1000),
          },
          fullDescription: {
            text:
              (f.description || f.title || '').substring(0, 1000) || 'Shieldly security finding.',
          },
          helpUri: 'https://www.shieldly.io/docs',
          help: {
            text: f.remediation || 'See the Shieldly analysis for remediation guidance.',
          },
          properties: {
      ...
      const messageParts = [f.title || f.description || 'Shieldly security finding'];
      if (f.description && f.title) messageParts.push(f.description);
      if (f.remediation) messageParts.push(`Remediation: ${f.remediation}`);
      if (f.resource) messageParts.push(`Resource: ${f.resource}`);
...
/**
 * Agent blast-radius mode — a thin wrapper over `shieldly agent-scan`.
 * Runs fully offline via the published CLI; no API key needed. Enforces the
 * repo's permission budget when budget-file is set.
 */
function runAgentScan() {
  const { spawnSync } = require('node:child_process');
  const budgetFile = core.getInput('budget-file').trim();
  const agentScanPath = core.getInput('agent-scan-path').trim();

  const cliArgs = ['-y', '@shieldly/cli@latest', 'agent-scan'];
  if (agentScanPath) cliArgs.push('--policy-file', agentScanPath);
  if (budgetFile) cliArgs.push('--budget', budgetFile);

  core.info('Shieldly Agent Blast-Radius scan (offline — nothing leaves the runner)');
  core.info(`npx ${cliArgs.join(' ')}`);
  const res = spawnSync('npx', cliArgs, { stdio: 'inherit', timeout: 300000 });
  if (res.error) {
    core.setFailed(`Shieldly agent-scan failed to start: ${res.error.message}`);
    return;
  }
  if (res.status !== 0) {
    core.setFailed(
      budgetFile
        ? `Shieldly agent-scan: permission budget "${budgetFile}" violated (see findings above).`
        : 'Shieldly agent-scan: CRITICAL or HIGH blast-radius findings (see above).'
    );
  }
}
...
async function run() {
  try {
    if (core.getInput('agent-scan').trim().toLowerCase() === 'true') {
      runAgentScan();
      return;
    }
    ...
    // SARIF report for GitHub code scanning (Security tab). Written even when
    // there are zero findings — an empty run tells code scanning the analysis
    // ran clean, which resolves previously-open alerts.
    const sarifFile = core.getInput('sarif-file').trim();
    if (sarifFile) {
      try {
        fs.writeFileSync(sarifFile, JSON.stringify(buildSarif(results), null, 2));
        core.setOutput('sarif-file', sarifFile);
        core.info(
          `Shieldly: SARIF report written to ${sarifFile} — upload with github/codeql-action/upload-sarif to see findings in the Security tab.`
        );
      } catch (sarifErr) {
        core.warning(`Shieldly: could not write SARIF file ${sarifFile} — ${sarifErr.message}`);
      }
    } else {
